...
def has_valid_entry_format(text):
    ...
    # Example: 挨 (rad.64) āi
    pattern1 = r'^[\u4e00-\u9fff][\u4e00-\u9fff\s]*\s*(\[[\u4e00-\u9fff\s]+\])?\s*\(rad\.\d+\)\s+[a-zA-Zāáǎàēéěèīíǐìōóǒòūúǔùüńňǹ]+'
    
    # Pattern 2: Romanization followed by Chinese characters  
    # Example: āi shēn pào 挨身炮
    pattern2 = r'^[a-zA-Zāáǎàēéěèīíǐìōóǒòūúǔùüńňǹ]+[\s]*[a-zA-Zāáǎàēéěèīíǐìōóǒòūúǔùüńňǹ]*\s+[\u4e00-\u9fff]+'
    
    # Pattern 3: Just Chinese characters (for simple entries)
    # Example: 八 (rad.12) bā
    pattern3 = r'^[\u4e00-\u9fff][\u4e00-\u9fff\s]*\s+[a-zA-Zāáǎàēéěèīíǐìōóǒòūúǔùüńňǹ]+'
